  ...
  return (
    <div className="px-4 py-12 mx-auto max-w-2xl">
      ...
      <div className="flex flex-col gap-4 mb-8">
        {startups.length === 0 && <div className="text-muted-foreground">You haven't listed any startups yet.</div>}
        {startups.map((startup) => (
          <Card key={startup.id} className="flex flex-col md:flex-row items-center gap-4 p-4">
            {startup.logo_url && (
              <img src={startup.logo_url} alt={startup.name} className="w-16 h-16 rounded object-cover bg-muted" />
            )}
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-lg">{startup.name}</span>
                <span title={startup.country} className="text-xl">
                  <ReactCountryFlag countryCode={startup.country} svg style={{ width: "1.2em", height: "1.2em", verticalAlign: "middle", borderRadius: "0.2em" }} />
                </span>
              </div>
              ...
              <div className="text-xs text-muted-foreground mt-1">Listed at: {new Date(startup.listed_at).toLocaleString()}</div>
            </div>
